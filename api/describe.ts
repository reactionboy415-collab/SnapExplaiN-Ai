...
// Simple in-memory rate limiter for Vercel serverless environment (per-instance fallback)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // Set CORS headers for Vercel proxy
  ...
  try {
    ...
    // Convert remote sample URL to base64 if needed
    if (base64Url.startsWith("http://") || base64Url.startsWith("https://")) {
      try {
        ...
        console.error("[SnapExplaiN Serverless] Error fetching remote image:", fetchErr);
    ...
    // Secure outbound request to external API
    ...
    if (!response.ok) {
      ...
      console.error(`[SnapExplaiN Serverless] AI endpoint returned status ${response.status}: ${errorText}`);
    ...
    console.error("[SnapExplaiN Serverless] Internal error processing request:", err);
